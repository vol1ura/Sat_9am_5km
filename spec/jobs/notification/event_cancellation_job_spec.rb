...
RSpec.describe Notification::EventCancellationJob do
  ...
  describe '#perform' do
    ...
    before { allow(Notification::User::Message).to receive(:call) }
    ...
    context 'when athletes with home event or going to event' do
      ...
      it 'sends notifications to users with telegram_id' do
        ...
        expect(Notification::User::Message).to have_received(:call).with(first_user, kind_of(String))
        expect(Notification::User::Message).to have_received(:call).with(second_user, kind_of(String))
        expect(Notification::User::Message).to have_received(:call).with(third_user, kind_of(String))
      end
    end
    ...
    context 'when no matching athletes' do
      it 'does not send any notifications' do
        described_class.perform_now event.id
        expect(Notification::User::Message).not_to have_received(:call)
      end
    end
  end
end
