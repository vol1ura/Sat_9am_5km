...
ActiveAdmin.register_page 'Dashboard' do
  ...
  content title: proc { t 'active_admin.dashboard' } do
    tabs do
      tab 'НОВОЕ' do
        panel 'Экспорт статистики для владельцев парков' do
          para 'Теперь владельцы парков могут экспортировать статистику по своим мероприятиям!'
          ul do
            li '📊 Выгрузка данных по мероприятию (результаты и волонтёрства)'
            li '👥 Отчёт по волонтёрским позициям'
            li '🏆 Отчёт по юбилеям (Milestones)'
            li '🆕 Отчёт по новичкам'
            li '🏃 Клубная статистика'
          end
          para 'Каждый отчёт можно скачать в формате CSV для дальнейшего анализа.'
          para link_to('Перейти к отчётам →', admin_utilities_path, class: 'button')
        end
      end

      tab t('active_admin.dashboard_welcome.info_tab', default: 'Информация') do
        columns do
          column do
          ...
          column do
            ...
            panel t('active_admin.dashboard_welcome.change_log') do
              ul do
                li 'На страницу участника добавлена таблица с возможными двойниками (поиск по имени и фамилии).'
                li 'Добавлена операция Удалить 🆙 в редактор протокола, позволяющая удалять лишние отсечки с пересчётом мест.'
                li 'Кнопка Опубликовать на странице забега. Если в забеге есть нестыковки, кнопка не отображается на панели.'
