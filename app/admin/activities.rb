...
ActiveAdmin.register Activity do
  ...
  filter :date
  filter :event, label: 'Забег', collection: -> { Event.authorized_for(current_user) }, as: :searchable_select
  ...
  controller do
    def destroy
      unless resource.published?
        resource.clear_live_results! if resource.token
        return super
      end
