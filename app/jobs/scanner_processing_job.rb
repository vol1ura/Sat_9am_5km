  ...
  def process_result(result_params)
    ...
    result.without_auditing do
      unless result.update(athlete:)
        ...
        Notification::ActivityAlertJob.perform_later @activity.id, %w[director results_handler], message
        Rollbar.warn 'Result processing failed', activity_id: @activity.id, message: message, errors: result.errors.inspect
      end
    end
  end
