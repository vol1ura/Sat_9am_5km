...
class Metrics::S95Collector < ApplicationService
  TTL = 5.minutes
  ...
  def collect_event_metrics
    events = Event.with_state.active
    events.each do |event|
      @data[:events_total] ||= {}
      @data[:events_total][event.country.code] = { active: event.active.to_s } unless @data[:events_total][event.country.code]
    end
  end
  ...
  def collect_activity_metrics
    Activity.published.includes(:event).find_each do |activity|
      ...
      @data[:activity_results_total][event_code][date_str] = activity.results.published.count
      ...
      @data[:activity_first_runs_total][event_code][date_str] = activity.results.published.where(first_run: true).count

      @data[:activity_personal_bests_total] ||= {}
      @data[:activity_personal_bests_total][event_code] ||= {}
      @data[:activity_personal_bests_total][event_code][date_str] = activity.results.published.where(personal_best: true).count
      ...
      @data[:activity_has_results][event_code][date_str] = activity.results.published.exists? ? 1 : 0
  ...
  def collect_athlete_metrics
    Event.active.find_each do |event|
      ...
      @data[:athletes_with_gender_total][event.code_name]['male'] = event.athletes.where(gender: 'male').count
      @data[:athletes_with_gender_total][event.code_name]['female'] = event.athletes.where(gender: 'female').count
  ...
  def collect_volunteer_metrics
    Activity.published.includes(:event).find_each do |activity|
      ...
      @data[:volunteer_roles_covered_total][event_code][date_str] = activity.volunteers.published.pluck(:role).uniq.count
  ...
  def calculate_average_time(activity)
    activity.results.published.where.not(total_time: nil).average(:total_time)&.to_i || 0
  end
  ...
  def calculate_median_time(activity)
    ...
    mid = count / 2
    if count.odd?
      results.mid(mid).first&.total_time || 0
    else
      (results.mid(mid - 1).first&.total_time.to_f + results.mid(mid).first&.total_time.to_f) / 2
    end.to_i
  end

  def calculate_best_time(activity)
    best_times = {}
    activity.results.published.where.not(total_time: nil).group(:athlete_id).minimum(:total_time).each do |athlete_id, time|
  ...
  def calculate_active_community(event)
    activity_ids = Activity.published.where(event: event).select(:id)
    result_athlete_ids = Result.where(activity: activity_ids).select(:athlete_id)
    volunteer_athlete_ids = Volunteer.where(activity: activity_ids).select(:athlete_id)
    result_athlete_ids.union(volunteer_athlete_ids).distinct.count
  end
  ...
  def format_metrics
    ...
    format_counter(metrics, 's95_activity_personal_bests_total', @data[:activity_personal_bests_total], %w[event activity_date])
    ...
    format_counter(metrics, 's95_athletes_with_external_code_total', @data[:athletes_with_external_code_total], %w[source])
    format_counter(metrics, 's95_athletes_going_to_event_total', @data[:athletes_going_to_event_total], %w[event])
    format_gauge(metrics, 's95_activity_average_time_seconds', @data[:activity_average_time_seconds], %w[event activity_date])
    format_gauge(metrics, 's95_activity_median_time_seconds', @data[:activity_median_time_seconds], %w[event activity_date])
    format_gauge(metrics, 's95_activity_best_time_seconds', @data[:activity_best_time_seconds], %w[event activity_date gender])
    ...
    format_counter(metrics, 's95_volunteers_by_role_total', @data[:volunteers_by_role_total], %w[event activity_date role])
    format_counter(metrics, 's95_unique_volunteers_total', @data[:unique_volunteers_total], %w[event window])
    format_gauge(metrics, 's95_volunteer_roles_covered_total', @data[:volunteer_roles_covered_total], %w[event activity_date])
    format_gauge(metrics, 's95_volunteer_position_coverage_ratio', @data[:volunteer_position_coverage_ratio], %w[event activity_date])
